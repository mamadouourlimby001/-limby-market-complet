...
export default function RenewServiceSubscriptionScreen() {
  const [form, setForm] = useState({ nomService: '', telephoneDepot: '', montant: '15000', serviceId: '' });
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get('/services/my-service').catch(() => ({ data: null }));
        if (res.data) {
          const service = res.data.service || res.data;
          setForm((prev) => ({ ...prev, nomService: service.nom, serviceId: service._id }));
        }
      } catch (err) {
        console.error(err);
      }
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/services/subscription-request', { ...form, montant: Number(form.montant) });
      setSuccess(true);
    } catch (err) {
  ...
  return (
    <Screen>
      ...
      <AlertBanner variant="info">
        Déposez 15 000 GNF sur le numéro Orange Money +224625223418 puis soumettez ce formulaire.
      </AlertBanner>
