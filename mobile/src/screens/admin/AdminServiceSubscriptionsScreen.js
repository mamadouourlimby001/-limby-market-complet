...
export default function AdminServiceSubscriptionsScreen() {
  const [requests, setRequests] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchRequests = async () => {
    try {
      const res = await api.get('/admin/service-subscription-requests');
  ...
  const handle = async (id, action) => {
    try {
      await api.post(`/admin/service-subscription-requests/${id}/${action}`);
      fetchRequests();
    } catch (err) {
  ...
  return (
    <Screen>
      <Text style={styles.title}>Demandes d'abonnement Services</Text>
      {requests.length === 0 ? (
        <EmptyState text="Aucune demande en attente" />
      ) : (
        requests.map(r => (
          <Card key={r._id} style={styles.card}>
            <Text style={styles.name}>{r.nomService}</Text>
