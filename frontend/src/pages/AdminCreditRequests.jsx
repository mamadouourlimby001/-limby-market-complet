...
const AdminCreditRequests = () => {
  ...
  const fetch = async () => {
    try { const res = await api.get('/admin/credit-requests'); setRequests(res.data); }
  ...
  return (
    <div className="page">
      <h1 className="page-title">Demandes de crédits</h1>
        ...
        requests.map(r => (
          <div key={r._id} className="card" style={{ padding: 12, marginBottom: 10 }}>
            <div style={{ marginBottom: 8 }}>
              <p style={{ fontSize: 14, fontWeight: 600 }}>{r.nomCompte}</p>
              ...
              <p style={{ fontSize: 12, color: '#6b7280' }}>→ Créditer: {r.telephoneCompte}</p>
