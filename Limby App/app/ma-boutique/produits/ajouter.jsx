...
import { BOUTIQUE_CATEGORIES } from '../../../constants/config';

export default function AjouterProduitBoutiqueScreen() {
  ...
  const handleSubmit = async () => {
    const { titre, categorie, prix, description } = form;
    if (!titre || !categorie || !prix || !description) {
    ...
    setLoading(true);
    try {
      // Get boutique ID first
      const boutiqueRes = await api.get('/boutiques/my-boutique');
      const boutiqueId = boutiqueRes.data.boutique?._id || boutiqueRes.data?._id;
      ...
      await api.post(`/boutiques/${boutiqueId}/products`, {
        titre,
        categorie,
        prix: Number(prix),
        description,
      ...
      Alert.alert('Succès !', 'Produit ajouté à votre boutique.', [
        { text: 'OK', onPress: () => router.back() },
      ]);
    } catch (err) {
      Alert.alert('Erreur', err.response?.data?.message || 'Impossible d\'ajouter le produit');
    } finally {
      setLoading(false);
    }
  };
...
const styles = StyleSheet.create({
  ...
  removePhoto: {
    ...
    backgroundColor: Colors.danger, width: 22, height: 22,
